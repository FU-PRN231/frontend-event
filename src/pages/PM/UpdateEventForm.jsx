...
const UpdateEventForm = () => {
  ...
  const onFinish = async (values) => {
    ...
    const data = await updateEvent(id, formData);
    if (data.isSuccess) {
      message.success("Chỉnh sửa sự kiện thành công");
    } else {
      message.error("Chỉnh sửa sự kiện không thành công");
    }
    setIsLoading(false);
  };
  ...
  const fetchData = async () => {
    ...
    if (data1.isSuccess && data2.isSuccess && data3.isSuccess) {
      ...
      setEventDetail(data3.result);
      await mapFieldToForm(data3.result);
    }
  };
  useEffect(() => {
    fetchData();
  ...
  const mapFieldToForm = async (eventDetail) => {
    debugger;
    const data = await getAllAvailableLocations("1970-01-01", "1970-01-01");
    if (data.isSuccess) {
      setLocation(data.result);
    }
    form.setFieldsValue({
      ...
      eventDate: [
        moment(eventDetail?.event?.startEventDate),
        moment(eventDetail?.event?.endEventDate),
      ],
      dateRange: [
        moment(eventDetail?.event?.startTime),
        moment(eventDetail?.event?.endTime),
      ],
      locationId: eventDetail?.event?.locationId,
      organizationId: eventDetail?.event?.organizationId,
      createSeatRankDtoRequests: eventDetail?.seatRanks?.map((item) => ({
        name: item.name,
        dateRange: [moment(item.startTime), moment(item.endTime)],
        price: item.price,
        quantity: item.quantity,
        description: item.description,
      })),
      sponsors: eventDetail?.eventSponsors?.map((item) => ({
      ...
      speakers: eventDetail?.speakers?.map((item) => ({
        name: item.name,
        description: item.description,
      })),
    });
